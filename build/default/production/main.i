...
# 875 "./mcc_generated_files/adcc.h"
...
int16_t sensorStatus = 0;
int16_t PreSensorStatus = 0;
...
# 122 "main.c"
        ...
        else if(referenceCorrection_time_cnt)
        {
            if(TimmingCnt.Flag._1sec)
            {
                if(referenceCorrection_time_cnt==12) referenceCorrection();
        ...
        else if(TimmingCnt.Flag._500ms) do { LATCbits.LATC1 = ~LATCbits.LATC1; } while(0);
...
void loadInitSetting(void)
{
    ...
    if(DATAEE_ReadByte(1)==0x3D)
    {
        SensorThreshold = (DATAEE_ReadByte(2)<<8) | DATAEE_ReadByte(3);
        threMin = (DATAEE_ReadByte(4)<<8) | DATAEE_ReadByte(5);
        threMax = (DATAEE_ReadByte(6)<<8) | DATAEE_ReadByte(7);
        varObsolescenceOut = (DATAEE_ReadByte(8)<<8) | DATAEE_ReadByte(9);
        referenceTemp = (DATAEE_ReadByte(10)<<8) | DATAEE_ReadByte(11);
        referenceBaseline = (DATAEE_ReadByte(12)<<8) | DATAEE_ReadByte(13);
        fRecoveryEnable = DATAEE_ReadByte(14);
        varRecoveryLimit = (DATAEE_ReadByte(15)<<8) | DATAEE_ReadByte(16);
        RecoveryCnt = DATAEE_ReadByte(17);
        RecoveryBaseline = (DATAEE_ReadByte(18)<<8) | DATAEE_ReadByte(19);
        detectTime = (DATAEE_ReadByte(20)<<8) | DATAEE_ReadByte(21);
        baseline_cor = (DATAEE_ReadByte(22)<<8) | DATAEE_ReadByte(23);
    }
    else
    {
        ...
        referenceTemp = 25;
        referenceBaseline = 13913;
        fRecoveryEnable = 1;
        varRecoveryLimit = 3000;
        RecoveryCnt = 0;
        fObsolescence = 0;
        RecoveryBaseline = 0;
        detectTime = 2;
...
void saveAllSetting(void)
{
    ...
    DATAEE_WriteByte(1, 0x3D);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(2, SensorThreshold>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(3, SensorThreshold);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(4, threMin>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(5, threMin);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(6, threMax>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(7, threMax);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(14, fRecoveryEnable);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(8, varObsolescenceOut>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(9, varObsolescenceOut);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(15, varRecoveryLimit>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(16, varRecoveryLimit);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(10, referenceTemp>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(11, referenceTemp);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(12, referenceBaseline>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(13, referenceBaseline);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(17, RecoveryCnt);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(18, RecoveryBaseline>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(19, RecoveryBaseline);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(20, detectTime>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(21, detectTime);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(22, ((uint16_t)(baseline_cor))>>8);
    while(NVMCON1bits.WR);

    DATAEE_WriteByte(23, (uint16_t)(baseline_cor));
    while(NVMCON1bits.WR);
...
void tempProc(void)
{
    ...
    tempData = TEMP_Read();
    if(tempData > 2047) tempData = (tempData - 4096);

    temperature = tempData >> 4;
    temperature_10 = 10*tempData >> 4;
}
...
void Reading_Get_Proc(void)
{
    ...
    {
        if(levelSensor.detect_time_cnt++ >= (100*detectTime))
        {
            levelSensor.detect_time_cnt = (100*detectTime);
...
void cmdProc(void)
{
    ...
    switch(idx)
    {
        case 0: bitMonitoring |= 0x01; break;
        case 1: bitMonitoring |= 0x02; break;
        case 2: bitMonitoring |= 0x04; break;
        case 3: bitMonitoring |= 0x10; break;
        case 4: bitMonitoring |= 0x20; break;
        case 5: bitMonitoring |= 0x100; break;
        case 6: printf("RD:%d\n\r", levelSensor.RD_data); break;
        case 7: printf("RB:%d\n\r", referenceBaseline); printf("RT:%d\n\r", referenceTemp); break;
        ...
        case 14: bitMonitoring |= 0x40; break;
        case 15: printf("TH:%d\n\r", (DATAEE_ReadByte(2)<<8) | DATAEE_ReadByte(3)); break;
        case 16: printf("OD:%d\n\r", varObsolescenceOut); break;
        case 17: varObsolescenceOut = p1;
                DATAEE_WriteByte(8, varObsolescenceOut>>8);
                DATAEE_WriteByte(9, varObsolescenceOut);
                break;
        case 18: printf("EE:%d,%d\n\r", p1, DATAEE_ReadByte(p1)); break;
        case 19: DATAEE_WriteByte(p1, p2); printf("eew %d %d\n\r", p1, p2); break;
        case 20: eepromDump(p1, p2); break;
        case 21: loadInitSetting(); break;
        case 22: saveAllSetting(); break;
        ...
        case 24: fRecoveryEnable = p1;
                DATAEE_WriteByte(14, fRecoveryEnable);
                break;
        case 25: printf("REB:%d\n\r", RecoveryBaseline);printf("REEN:%d\n\r", fRecoveryEnable); break;
        case 26: printf("TE:%d\n\r", temperature); break;
        case 27: bitMonitoring |= 0x80; break;
        case 28: threMin = p1;
                DATAEE_WriteByte(4, threMin>>8);
                DATAEE_WriteByte(5, threMin);
                break;
        ...
        case 37: detectTime = p1;
            DATAEE_WriteByte(20, detectTime>>8);
            DATAEE_WriteByte(21, detectTime);
        case 38: printf("BC:%d\n\r", (uint16_t)(baseline_cor));
            break;
        case 39: baseline_cor = p1;
            DATAEE_WriteByte(22, baseline_cor>>8);
            DATAEE_WriteByte(23, baseline_cor);
            break;
    }
}
...
void setThresholdLevel(void)
{
    ...
    a = (double)(threMax - threMin) / (double)(1023 - 0);
    b = (double)threMin - a * (double)0;
...
void AVG_Fitering(AVGF_t *_data, int16_t _inputData)
{
 ...
 if(_data->avg_filtering_cnt >= 5) _data->avg_filtering_cnt = 5;
 ...
 *(_data->data_buf+5 -1) = _data->input_data;

 if(_data->avg_filtering_cnt >= 5)
 {
  _data->fitering_data = (int16_t)(_data->data_sum / (5 -2));
 }
